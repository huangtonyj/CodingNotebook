...
// find partial, where is a string denoting a partial name to search the application
// for.It must count the number of contacts starting with and print the count on a new line.
...
class Contact {
  ...
  find(partial) {
    ...
    for (let contact of this.contacts) {
      count += contact.indexOf(partial) === 0 ;
    }    
    return count;
  }
}
...
const c = new Contact();
  c.add('hack');
  c.add('hackerrank');
  console.log(c.find('hac'));
  console.log(c.find('hak'));