/*
  ...
  In a Unix-like operating system, a path is bound by the following rules:
    ...
    a path starts with /, it's an absolute path; if it
...
function shortenPath(path) {
  ...
  if (absolutePath) shortenedPath.push('');
  
  path.split('/').forEach((dir) => {
    if (dir === '..') {
      const isPrevParent = shortenedPath[shortenedPath.length - 1] === '..';
      const isPrevAbsolutePath = shortenedPath[shortenedPath.length - 1] !== '';
      
      if (shortenedPath.length === 0 || isPrevParent) {
        shortenedPath.push(dir);
      } else if (isPrevAbsolutePath) {
        shortenedPath.pop();
      }
  ...
  if (shortenedPath.length === 1 && shortenedPath[0] === '') return '/';
...
// console.log(
//   shortenPath('/foo/../test/../test/../foo//bar/./baz'), '/foo/bar/baz', '\n',
//   shortenPath('../../foo/../../bar/baz'), '../../../bar/baz', '\n',
//   shortenPath('/../../foo/bar/baz'), '/foo/bar/baz', '\n',
//   shortenPath('/../../../this////one/./../../is/../../going/../../to/be/./././../../../just/a/forward/slash/../../../../../..'), '/', '\n',
// );