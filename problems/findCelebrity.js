...
const graph = [[1,1,0],[0,1,0],[1,1,1]];

function knows(personA, personB) {
  return graph[personA][personB];
}
...
function findCelebrity(n) {
  ...
  // compare two people and skip person that is not celebrity candidate
  while (a < n && b < n) {
    if (knows(a, b)) {
      a = Math.max(a, b) + 1;
    } else {
  ...
  // last person standing is candidate celebrity
  const candidateCelebrity = a < n ? a : b;
  
  // Double check candidateCelebrity is celebrity;
  for (let i = 0; i < n; i++) {
    if (i === candidateCelebrity) continue;
    
    if (!knows(i, candidateCelebrity)) return -1;
    if (knows(candidateCelebrity, i)) return -1;
  }
